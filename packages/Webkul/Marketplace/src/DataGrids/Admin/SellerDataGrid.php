    ...
    public function prepareQueryBuilder()
    {
        if ( (core()->getConfigData('marketplace.settings.seller_flag.enable'))) {
            $queryBuilder = DB::table('marketplace_sellers')
                    ...
                    ->select('marketplace_sellers.id', 'marketplace_sellers.created_at', 'customers.email', 'marketplace_sellers.is_approved', 'marketplace_sellers.customer_id', DB::raw('CONCAT(customers.first_name, " ", customers.last_name) as customer_name') )->selectRaw('count(marketplace_seller_flags.id) as flags')
